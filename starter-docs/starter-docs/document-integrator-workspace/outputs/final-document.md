...
These constraints govern the entire project and must never be violated:

1. **Non-destructive editing is non-negotiable.** All visual changes operate on a temporary overlay layer. Original source files remain unmodified until the user explicitly saves.
2. **The dual AI system is mandatory.** Both the inline element AI agents AND the persistent left-sidebar project-wide chat must be implemented as distinct interfaces -- never collapsed into one.
3. **Never store API keys in plaintext.** User-provided API keys are the sole authentication method and must be encrypted via the platform system keychain at all times.
4. **File system access is sandboxed.** All file paths must be validated against the user-granted project scope before any I/O operation.
5. **Iframe preview isolation is required.** The rendered preview uses an iframe with `postMessage` communication -- Shadow DOM is insufficient because it does not isolate JavaScript execution.

---

## Executive Summary

You are building a **Tauri 2.0 desktop application** with a **Rust backend** and a **React/TypeScript frontend**. The application is a visual HTML editor with AI integration, structured around a **four-panel layout** (left sidebar for AI chat, center panel for visual preview, right sidebar for property inspection, bottom panel for file navigation). Its core differentiator is a **dual AI system**: inline element agents for scoped edits and a persistent project-wide chat for multi-file operations. The key architectural pillars are: **non-destructive editing** via a change buffer, **user-provided API keys** stored in the system keychain, **iframe-based preview isolation** with postMessage communication, **explicit file consent** before any I/O, and a **plugin-extensible** AI provider system. The application must build and run on Windows, macOS, and Linux.

---

## Phase 0 -- Implementation Guidance: Response Format and Session Management

**This section governs how you produce output. Read it before writing any code.**

This application is too large to produce in a single response. It is a **multi-session project**. Do not attempt to generate everything at once. Instead, follow these rules:

### 0.1 Session Structure
- **Each session should produce one coherent implementation layer** that builds on the previous session's output.
- Follow the implementation roadmap in section 4.2 as the session sequence:
  - **Session 1:** Level 1 -- Foundation (project setup, Rust backend file operations, path validation, four-panel layout shell, state store, API key management)
  - **Session 2:** Level 2 -- Visual Editor (iframe preview, element overlay, non-destructive editing layer, CSS variable engine, color picker, image pipeline, Monaco code view, undo/redo)
  - **Session 3:** Level 3 -- AI Integration (provider adapters, context extraction, inline agent, project chat with streaming, orchestration)
  - **Session 4:** Level 4 -- Polish and Distribution (onboarding, keyboard shortcuts, export, Git integration, testing, accessibility, cross-platform packaging)

### 0.2 Output Format
- Produce **complete, runnable files** with full filenames (e.g., `src-tauri/src/commands/file_ops.rs`). Do not produce skeleton code, pseudocode, or abbreviated implementations.
- Use the deliverables list in section 4.1 as the output structure.
- Each file must be self-contained and functional when placed at its specified path in the project directory tree (section 2.2).

### 0.3 Continuation Protocol
- At the end of each session, state what was completed and what the next session should produce.
- When a follow-up prompt says "continue" or "next layer," proceed to the next level in the roadmap.
- If a follow-up prompt requests a specific feature or fix, address it within the context of the current implementation layer.
- Always reference the section numbers from this specification when explaining implementation decisions.

### 0.4 Quality Gate
- Before concluding any session, verify the produced code against the relevant items in the verification checklist (section 4.5) and the critical constraints listed at the top of this document.
...
- **CORS policies must be enforced** for all external resource loading (images, fonts, stylesheets referenced by user projects). Configure the Tauri HTTP plugin and CSP to allow only explicitly approved external origins. When the preview iframe loads external resources, validate the origin against a user-configurable allowlist.
...
src-tauri/
  src/
    ...
    plugins/
      mod.rs             -- Plugin registry, loader, and trait definitions
      plugin_manifest.rs -- Plugin manifest parsing and validation
    lib.rs               -- Module declarations
  plugins/               -- Directory for installed plugin packages
  Cargo.toml
...
src/
  ...
  components/
    ...
    editor/
      ...
      ViewportSelector.tsx -- Responsive preview viewport controls
    ai/
  ...
  hooks/
    ...
    usePlugins.ts        -- Plugin discovery and management
  store/
  ...
  utils/
    ...
    logger.ts            -- Structured logging utility for frontend
  types/
    index.ts             -- Shared TypeScript type definitions
    plugin.ts            -- Plugin interface type definitions
...
|  - Provider      |  - Responsive viewport    |  - Element tree  |
|    selector      |    controls               |    path          |
|                  |                           |                  |
+------------------+---------------------------+------------------+
|                       BOTTOM PANEL                              |
...
- **Center Panel:** The visual editor showing an iframe-rendered preview of the active HTML file. An overlay system provides element highlighting on hover and selection indicators on click. Inline AI agent popovers appear near selected elements. A **responsive viewport toolbar** at the top of the panel provides device-size previews (see section 2.4.5).
...
#### 2.4.5 Responsive Preview Modes
The Center Panel includes a **viewport toolbar** above the iframe preview that allows users to preview their HTML at different device sizes:

- **Preset viewports:**
  - Mobile: 375px wide (iPhone SE / standard mobile)
  - Tablet: 768px wide (iPad / standard tablet)
  - Desktop: 1440px wide (standard desktop monitor)
- **Custom dimensions:** A width x height input field allowing the user to enter any arbitrary viewport size
- **Fit-to-panel scaling:** When the selected viewport width exceeds the available panel width, the preview automatically scales down (using CSS `transform: scale()`) to fit within the panel while maintaining the correct aspect ratio. A zoom percentage indicator shows the current scale factor.
- **Orientation toggle:** For mobile and tablet presets, a button to swap width and height (portrait/landscape)
- The iframe's `width` and `height` are set to the selected viewport dimensions. The preview content renders as it would in a browser window of that size, triggering the project's responsive CSS breakpoints naturally.
...
The application must support extensibility through a plugin architecture, enabling custom AI providers beyond the three built-in adapters and custom editing tools beyond the default set.

#### 2.11.1 Plugin Interface for AI Providers
Extend the `AIProvider` trait (section 2.5.1) to support dynamically loaded providers:

```rust
// Plugin-provided AI providers implement the same AIProvider trait
// and are loaded via a manifest file and dynamic dispatch
...
Third-party AI providers (e.g., Ollama for local models, Cohere, Mistral) can implement the `AIProvider` trait and package themselves as a plugin. The application loads them at startup from the `src-tauri/plugins/` directory.

#### 2.11.2 Extension Points for Custom Editing Tools
Provide a frontend plugin API that allows custom editing tools to register themselves:
- Custom context menu actions on selected elements
- Custom property inspector panels (e.g., a specialized animation editor)
- Custom export formats beyond the built-in options

The frontend exposes a `registerPlugin(manifest, handlers)` API that tools can call to inject themselves into the UI at defined extension points.

#### 2.11.3 Plugin Registration and Loading
- At application startup, scan the `src-tauri/plugins/` directory for plugin manifest files (`plugin.toml` or `plugin.json`)
- Validate each manifest for required fields (name, version, capabilities, entry point)
- Load valid plugins and register their capabilities with the appropriate subsystems (AI provider registry, editing tool registry, export format registry)
- Plugins that fail to load produce a warning log entry but do not prevent application startup
- The Settings UI includes a "Plugins" tab showing installed plugins, their status, and an enable/disable toggle

### 2.12 Logging and Debugging Infrastructure

Implement a comprehensive, structured logging system across both the Rust backend and the React frontend:

#### 2.12.1 Structured Logging
- Use severity levels throughout: **debug**, **info**, **warn**, **error**
- All log entries include: timestamp (ISO 8601), severity level, source module, and a structured message
- Rust backend uses the `tracing` crate (or `log` + `env_logger`) with structured fields for key operations (file path, command name, provider, duration)
- Frontend uses a shared `logger.ts` utility that formats messages consistently and forwards critical logs (warn, error) to the Rust backend via an IPC command for unified log output

#### 2.12.2 Log Categories
- **File operations:** Log all read, write, create, and delete operations with the target path and outcome (success/failure)
- **AI requests:** Log provider, request type (inline/chat), token count estimate, response time, and success/failure status. Do NOT log prompt content or API keys.
- **Security events:** Log path validation failures, permission denials, and sanitization actions
- **Plugin lifecycle:** Log plugin load, registration, enable/disable, and errors
- **User actions:** Log significant UI actions at debug level (project open, file select, save, export) for diagnostic replay

#### 2.12.3 Log Rotation and Size Management
- Log files are stored in the Tauri app data directory (`tauri::api::path::app_log_dir`)
- Implement log rotation: create a new log file daily or when the current file exceeds 10MB
- Retain logs for the most recent 7 days (configurable in settings). Automatically delete older log files.
- Total log storage is capped at 100MB -- oldest files are purged when the cap is reached

#### 2.12.4 Debug Mode
- A "Debug Mode" toggle in the application settings enables verbose (debug-level) logging output
- When Debug Mode is active, the Bottom Panel's console tab shows a live-streaming log view with filtering by severity and module
- Debug Mode also enables additional diagnostic overlays in the visual editor (e.g., element bounding box coordinates, postMessage traffic)

#### 2.12.5 Error Reporting and Crash Diagnostics
- Unhandled panics in the Rust backend are caught, logged with full stack traces, and surfaced to the user as a recovery dialog ("The application encountered an error. Would you like to restart or view the error log?")
- Unhandled JavaScript exceptions are caught by a global error boundary, logged with component stack traces, and displayed with a "Report Bug" option that copies the relevant log excerpt to the clipboard
- On application startup, check for crash logs from the previous session and offer to send a diagnostic report (with user consent)
...
- [ ] CORS policies are enforced for external resource loading in the preview iframe -- origins are validated against a user-configurable allowlist
- [ ] File system access auditing: all write operations are logged with structured severity and path information (see section 2.12)
...
- [ ] CSP is configured and restrictive, including CORS-aware rules for external resource origins
...
2. **Configuration files:**
   ...
   - `Cargo.toml` with all Rust dependencies (tauri, serde, tokio, reqwest, keyring, tracing, etc.)
   - `package.json` with all frontend dependencies (React, TypeScript, Tailwind, Monaco Editor, Zustand, etc.)
   - `tsconfig.json` with strict TypeScript configuration
3. **Complete Rust backend** -- all commands, handlers, AI provider adapters, path validation, credential management, plugin loader, logging infrastructure, and file operations with full error handling
...
5. **Visual editor implementation** -- the iframe preview, element overlay, click-to-edit system, responsive viewport controls, color picker with CSS variable awareness, and image handling pipeline
...
7. **Plugin system** -- the plugin manifest format, loader, registry, and at least one example plugin demonstrating the AI provider extension point
8. **File system management** -- safe file operations, watcher, Git integration, backup creation, and permission enforcement
9. **Testing suite** -- unit tests for Rust backend, integration tests for IPC, and at least one end-to-end test for a core workflow
10. **Step-by-step build and deployment instructions** -- from `npm install` through `cargo tauri build`, including code signing setup for distribution
11. **User documentation** -- setup guide covering API key configuration, first project opening, and basic editing workflows
12. **Developer documentation** -- architecture decisions and rationale, extension points and plugin API reference, contribution guidelines for future maintainers
...
- Set up structured logging infrastructure (section 2.12)
...
- Build the responsive viewport toolbar with preset and custom sizes (section 2.4.5)
...
- Build the plugin system and plugin loader (section 2.11)
...
- Finalize developer documentation and plugin API reference
...
| Extensibility model | Plugin architecture with trait-based dispatch | Enables third-party AI providers and editing tools without modifying core application code |
...
- [ ] Responsive viewport preview modes work for mobile, tablet, and desktop sizes
- [ ] Explicit consent dialog appears during project opening
- [ ] User-provided API keys are the only authentication method
- [ ] The plugin system loads and registers at least one provider plugin
...
- [ ] Structured logging is operational with rotation and debug mode toggle
